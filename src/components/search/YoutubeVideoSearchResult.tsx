...
export const YoutubeVideoSearchResult: React.FC<Props> = ({item, playVideo}: Props) => {
    ...
    const typescriptMonads = require("typescript-monads");

    const handleClick = () => {
        new typescriptMonads.Maybe(item.id).map((id: NonNullable<gapi.client.youtube.ResourceId>) => {
            playVideo(id);
            return id;
        })
    }
    ...
    const handleAddToQueue = () => {
        new typescriptMonads.Maybe(item.id?.videoId).map((id: NonNullable<string>) => {
            getYoutubeVideo(id)
                .then((res: AxiosResponse<gapi.client.youtube.VideoListResponse>) => {
